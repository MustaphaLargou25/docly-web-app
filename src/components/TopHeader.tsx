    ...
    <header className="hidden md:flex fixed top-0 inset-x-0 z-20 h-16 items-center gap-4 px-5 bg-background/90 backdrop-blur border-b border-divider ps-[236px]">
      ...
      >
        <div className="relative">
          ...
          <label htmlFor="docly-header-search" className="sr-only">{t("search")}</label>
          <input
            id="docly-header-search"
            type="search"
            value={query}
          ...
          <DropdownMenuTrigger asChild>
            <button className="h-10 w-10 rounded-full overflow-hidden" aria-label="Profile menu">
              <Avatar className="h-10 w-10">
                <AvatarFallback className="bg-primary text-primary-foreground text-sm font-semibold">{profile.initials}</AvatarFallback>
              </Avatar>
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56">
            <div className="px-2 py-2">
              <div className="text-[13px] font-semibold">{profile.displayName || "—"}</div>
